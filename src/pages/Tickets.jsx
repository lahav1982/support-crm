...
export default function Tickets({ tickets, setTickets }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ width: 320, background: "#fff", borderRight: "1px solid #EAECF0", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "14px 16px 10px", borderBottom: "1px solid #F3F4F6" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#1C2B1C" }}>All Tickets</span>
            {openCount > 0 && (
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
            {["all", "open", "in progress", "resolved"].map(s => (
              <button key={s} onClick={() => setFilter(s)}
                style={{ padding: "5px 8px", fontSize: 12, fontWeight: filter === s ? 700 : 500, color: filter === s ? "#3D6B3D" : "#8A9E8A", background: filter === s ? "#EAF0EA" : "transparent", border: "none", borderRadius: 6, cursor: "pointer", textTransform: "capitalize", whiteSpace: "nowrap" }}>
                {s}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {["all", "high", "medium", "low"].map(p => (
              <button key={p} onClick={() => setPriorityFilter(p)}
                style={{ padding: "4px 8px", fontSize: 12, fontWeight: priorityFilter === p ? 700 : 500, color: priorityFilter === p ? (PRIORITY_COLORS[p]?.text || "#3D6B3D") : "#8A9E8A", background: priorityFilter === p ? (PRIORITY_COLORS[p]?.bg || "#EAF0EA") : "transparent", border: "none", borderRadius: 6, cursor: "pointer", textTransform: "capitalize" }}>
                {p}
              </button>
            ))}
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "0 8px 8px" }}>
          {ticketItems.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 20px", color: "#8A9E8A" }}>
            ...
            <div style={{ textAlign: "center", padding: "32px 16px", color: "#8A9E8A", fontSize: 15 }}>No tickets match filters</div>
          ) : filtered.map(ticket => {
            const tc = TAG_COLORS[ticket.tag] || { bg: "#F0EDE6", text: "#6B7D6B", dot: "#8A9E8A" };
            ...
            return (
              ...
              <div onClick={() => setSelected(ticket)}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#1C2B1C", flex: 1, paddingRight: 8, lineHeight: 1.4 }}>{ticket.subject}</span>
                  <span style={{ fontSize: 12, color: "#8A9E8A", whiteSpace: "nowrap" }}>{ticket.date}</span>
                </div>

                {/* Customer */}
                <div style={{ fontSize: 13, color: "#6B7D6B", marginBottom: 6 }}>{ticket.customerName}</div>

                {/* Summary preview */}
                <div style={{ fontSize: 13, color: "#8A9E8A", marginBottom: 8, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{ticket.body}</div>
      ...
      {/* Right: ticket detail */}
      {selected ? (
        <div style={{ flex: 1, overflowY: "auto", background: "#F7F5F0", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 13, background: "#EAF0EA", color: "#3D6B3D", fontWeight: 700, borderRadius: 6, padding: "2px 8px" }}>🎫 Ticket #{selected.id}</span>
                  {saving && <span style={{ fontSize: 13, color: "#8A9E8A" }}>Saving…</span>}
                </div>
                <h2 style={{ fontSize: 19, fontWeight: 800, color: "#1C2B1C", marginBottom: 8, letterSpacing: "-0.4px" }}>{selected.subject}</h2>
          ...
          <div style={{ flex: 1, padding: "24px 28px", display: "flex", gap: 20 }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#8A9E8A", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>Summary</div>
              ...
              {selected.notes?.startsWith("Action needed:") && (
                <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 12, padding: "16px 20px" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 15.5, color: "#3A4E3A", fontWeight: 600 }}>{selected.notes.split("\n")[0].replace("Action needed: ", "")}</p>
                </div>
              )}
              ...
              {selected.replies?.length > 0 && (
                <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#8A9E8A", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 12 }}>Activity</div>
                  {selected.replies.map((r, i) => (
                    <div key={i} style={{ borderLeft: "3px solid #6366F1", paddingLeft: 14, marginBottom: 12 }}>
                      <div style={{ fontSize: 13, color: "#8A9E8A", marginBottom: 4 }}><span style={{ color: "#3D6B3D", fontWeight: 700 }}>{r.author}</span> · {new Date(r.timestamp).toLocaleDateString()}</div>
                      <p style={{ margin: 0, fontSize: 15, color: "#3A4E3A", lineHeight: 1.6 }}>{r.body}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {/* Internal notes */}
              <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#8A9E8A", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>Internal Notes</div>
                <textarea
                  ...
                  style={{ width: "100%", minHeight: 100, resize: "vertical", background: "#FFFBEB", border: "1.5px solid #FDE68A", borderRadius: 9, color: "#3A4E3A", padding: "12px", fontSize: 15, outline: "none", fontFamily: "inherit", lineHeight: 1.6 }}
                />
              </div>
            </div>
            ...
            <div style={{ width: 220, display: "flex", flexDirection: "column", gap: 16, flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {STATUSES.map(s => (
                    <button key={s} onClick={() => handleField("status", s)}
                      style={{ padding: "8px 12px", fontSize: 14, fontWeight: selected.status === s ? 700 : 500, borderRadius: 8, border: selected.status === s ? "none" : "1.5px solid #E5E7EB", background: selected.status === s ? statusColor(s).bg : "#fff", color: selected.status === s ? statusColor(s).text : "#6B7D6B", cursor: "pointer", textAlign: "left", textTransform: "capitalize" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {TEAM.map(m => (
                    <button key={m.id} onClick={() => handleField("assignedTo", m.id)}
                      style={{ padding: "8px 12px", fontSize: 14, fontWeight: selected.assignedTo === m.id ? 700 : 500, borderRadius: 8, border: selected.assignedTo === m.id ? "none" : "1.5px solid #E5E7EB", background: selected.assignedTo === m.id ? "#EAF0EA" : "#fff", color: selected.assignedTo === m.id ? "#3D6B3D" : "#6B7D6B", cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <SideCard label="Category">
                <select value={selected.tag} onChange={e => handleField("tag", e.target.value)}
                  style={{ width: "100%", background: "#FAFAF7", border: "1.5px solid #E5E7EB", borderRadius: 8, color: "#1C2B1C", padding: "9px 10px", fontSize: 14, fontWeight: 600, outline: "none", cursor: "pointer" }}>
              ...
              <SideCard label="Details">
                <div style={{ fontSize: 14, color: "#6B7D6B", lineHeight: 2 }}>
                  <div>Created: <span style={{ color: "#1C2B1C", fontWeight: 600 }}>{selected.date}</span></div>
                  <div>Customer: <span style={{ color: "#1C2B1C", fontWeight: 600 }}>{selected.customerName}</span></div>
                </div>
              </SideCard>
            </div>
          </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, background: "#F7F5F0" }}>
          <div style={{ fontSize: 40 }}>🎫</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#6B7D6B" }}>Select a ticket to view details</div>
          <div style={{ fontSize: 14, color: "#8A9E8A", maxWidth: 280, textAlign: "center", lineHeight: 1.6 }}>
...
function SideCard({ label, children }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "14px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#8A9E8A", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>{label}</div>
      {children}
    </div>
  );
}
...
function statusColor(status) {
  ...
  if (status === "resolved")    return { bg: "#EAF0EA", text: "#3D6B3D" };
  return { bg: "#F0EDE6", text: "#6B7D6B" };
}
