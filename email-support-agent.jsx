...
export default function EmailSupportAgent() {
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "20px 28px", background: "#fff", overflow: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                ...
                <button
                  ...
                  disabled={loading}
                  style={{ background: loading ? "#e0dfff" : "linear-gradient(135deg, #6c63ff, #48c6ef)", color: loading ? "#9090c0" : "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 8, transition: "all 0.2s" }}>
                  {loading ? (
                    <>
                      <span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #9090c0", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
                      Generating...
                    </>
                  ) : "✦ Generate AI Reply"}
                </button>
              ...
              {sent ? (
                ...
                <>
                  <textarea
                    ...
                    style={{ flex: 1, minHeight: 180, resize: "none", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "14px 16px", fontSize: 13.5, color: "#374151", outline: "none", lineHeight: 1.7, fontFamily: "inherit", transition: "border 0.15s" }}
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12, gap: 10 }}>
                    ...
                    <button onClick={sendReply} disabled={!draft.trim()} style={{ background: draft.trim() ? "#1a1a2e" : "#e5e7eb", color: draft.trim() ? "#fff" : "#9ca3af", border: "none", borderRadius: 8, padding: "9px 22px", fontSize: 13, fontWeight: 700, cursor: draft.trim() ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
                      Send Reply ↑
                    </button>
                  </div>
                </>
