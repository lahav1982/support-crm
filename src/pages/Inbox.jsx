...
export default function Inbox({ tickets, setTickets, businessContext }) {
  ...
  // Send reply but keep ticket OPEN
  ...
  // Summarize conversation into a ticket using AI
  ...
  // Create a new ticket from the AI summary
    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ width: 290, background: "#fff", borderRight: "1px solid #EAECF0", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "0 8px 8px" }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 16px", color: "#9CA3AF", fontSize: 13 }}>No {tab === "all" ? "" : tab} tickets</div>
          ) : filtered.map(ticket => {
      ...
      {selected ? (
        <>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#F5F6FA" }}>
            ...
            {showSummarizer && (
              <div style={{ background: "#F5F3FF", borderBottom: "1px solid #DDD6FE", padding: "16px 24px" }}>
                    ...
                    {!ticketCreated ? (
                      <button onClick={handleCreateTicket} disabled={creatingTicket}
                        ...
                        {creatingTicket
                          ...
                          : <><TicketIcon />Create Ticket from This Conversation</>}
                      </button>
                    ) : (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#16A34A", fontSize: 13, fontWeight: 700 }}>
                        ...
                        Ticket created and added to inbox!
                      </div>
                    )}
                  </div>
                )}
                ...
                {summary?.error && (
                  <div style={{ color: "#EF4444", fontSize: 13 }}>Error: {summary.error}</div>
                )}
              </div>
            )}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "16px 24px", background: "#F5F6FA", overflow: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button onClick={handleGenerate} disabled={loading}
                  ...
                  {loading
                    ? <><span style={{ width: 11, height: 11, border: "2px solid #9CA3AF", borderTopColor: "transparent", borderRadius: "50%", display: "inline-block", animation: "spin 0.7s linear infinite" }} />Generating…</>
                    : <>✦ Generate AI Reply</>}
                </button>
              </div>

              {replySent && (
                <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 8, padding: "8px 14px", marginBottom: 10, fontSize: 12, color: "#16A34A", fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10, gap: 8 }}>
                ...
                <button onClick={handleSendReply} disabled={!draft.trim() || saving}
                  style={{ background: draft.trim() ? "#fff" : "#F9FAFB", color: draft.trim() ? "#6366F1" : "#9CA3AF", border: `1.5px solid ${draft.trim() ? "#DDD6FE" : "#E5E7EB"}`, borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: draft.trim() ? "pointer" : "not-allowed", transition: "all 0.1s" }}>
                  {saving ? "Saving…" : "Send Reply"}
                </button>
                <button onClick={handleSendAndResolve} disabled={!draft.trim() || saving}
                  style={{ background: draft.trim() ? "#0F1117" : "#E5E7EB", color: draft.trim() ? "#fff" : "#9CA3AF", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 12, fontWeight: 700, cursor: draft.trim() ? "pointer" : "not-allowed", transition: "all 0.1s" }}>
          ...
          <div style={{ width: 240, background: "#fff", borderLeft: "1px solid #EAECF0", padding: 18, overflowY: "auto", display: "flex", flexDirection: "column", gap: 22 }}>
            ...
            <MetaSection label="Ticket Status">
