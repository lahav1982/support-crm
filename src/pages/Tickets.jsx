    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ width: 320, background: "#fff", borderRight: "1px solid #EAECF0", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "14px 16px 10px", borderBottom: "1px solid #F3F4F6" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#0F1117" }}>All Tickets</span>
            {openCount > 0 && (
              <span style={{ background: "#FEF2F2", color: "#EF4444", fontSize: 10, fontWeight: 700, borderRadius: 20, padding: "2px 8px" }}>{openCount} open</span>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
            {["all", "open", "in progress", "resolved"].map(s => (
              <button key={s} onClick={() => setFilter(s)}
                style={{ padding: "5px 8px", fontSize: 10, fontWeight: filter === s ? 700 : 500, color: filter === s ? "#6366F1" : "#9CA3AF", background: filter === s ? "#F0EFFE" : "transparent", border: "none", borderRadius: 6, cursor: "pointer", textTransform: "capitalize", whiteSpace: "nowrap" }}>
                {s}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {["all", "high", "medium", "low"].map(p => (
              <button key={p} onClick={() => setPriorityFilter(p)}
                style={{ padding: "4px 8px", fontSize: 10, fontWeight: priorityFilter === p ? 700 : 500, color: priorityFilter === p ? (PRIORITY_COLORS[p]?.text || "#6366F1") : "#9CA3AF", background: priorityFilter === p ? (PRIORITY_COLORS[p]?.bg || "#F0EFFE") : "transparent", border: "none", borderRadius: 6, cursor: "pointer", textTransform: "capitalize" }}>
                {p}
              </button>
            ))}
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "8px" }}>
          {ticketItems.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 20px", color: "#9CA3AF" }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>No tickets yet</div>
              <div style={{ fontSize: 12, lineHeight: 1.6 }}>Convert a customer conversation to a ticket using the <strong>"Convert to Ticket"</strong> button in the Inbox.</div>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "32px 16px", color: "#9CA3AF", fontSize: 13 }}>No tickets match filters</div>
          ) : filtered.map(ticket => {
            ...
            return (
              <div key={ticket.id} onClick={() => setSelected(ticket)}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#0F1117", flex: 1, paddingRight: 8, lineHeight: 1.4 }}>{ticket.subject}</span>
                  <span style={{ fontSize: 10, color: "#9CA3AF", whiteSpace: "nowrap" }}>{ticket.date}</span>
                </div>

                {/* Customer */}
                <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 6 }}>{ticket.customerName}</div>

                {/* Summary preview */}
                <div style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 8, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{ticket.body}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                  ...
                  <span style={{ background: pc.bg, color: pc.text, fontSize: 10, fontWeight: 600, borderRadius: 6, padding: "2px 7px", textTransform: "capitalize" }}>{ticket.priority}</span>
                  <span style={{ background: statusColor(ticket.status).bg, color: statusColor(ticket.status).text, fontSize: 10, fontWeight: 600, borderRadius: 6, padding: "2px 7px", textTransform: "capitalize" }}>{ticket.status}</span>
                  {assignee && (
                    <span style={{ marginLeft: "auto", width: 20, height: 20, borderRadius: "50%", background: assignee.color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 9, fontWeight: 800 }}>{assignee.avatar}</span>
                  )}
                </div>
              </div>
            );
      ...
      {selected ? (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 11, background: "#F0EFFE", color: "#6366F1", fontWeight: 700, borderRadius: 6, padding: "2px 8px" }}>🎫 Ticket #{selected.id}</span>
                  {saving && <span style={{ fontSize: 11, color: "#9CA3AF" }}>Saving…</span>}
                </div>
                <h2 style={{ fontSize: 17, fontWeight: 800, color: "#0F1117", marginBottom: 8, letterSpacing: "-0.4px" }}>{selected.subject}</h2>
          ...
          <div style={{ flex: 1, padding: "24px 28px", display: "flex", gap: 20 }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>Summary</div>
              ...
              {selected.notes?.startsWith("Action needed:") && (
                <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 12, padding: "16px 20px" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                    <span>⚡</span> Action Needed
                  </div>
                  <p style={{ margin: 0, fontSize: 13.5, color: "#374151", fontWeight: 600 }}>{selected.notes.split("\n")[0].replace("Action needed: ", "")}</p>
                </div>
              )}
              ...
              {selected.replies?.length > 0 && (
                <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 12 }}>Activity</div>
                  {selected.replies.map((r, i) => (
                    <div key={i} style={{ borderLeft: "3px solid #6366F1", paddingLeft: 14, marginBottom: 12 }}>
                      <div style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 4 }}><span style={{ color: "#6366F1", fontWeight: 700 }}>{r.author}</span> · {new Date(r.timestamp).toLocaleDateString()}</div>
              ...
              {/* Internal notes */}
              <div style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>Internal Notes</div>
                <textarea
                  ...
                  style={{ width: "100%", minHeight: 100, resize: "vertical", background: "#FFFBEB", border: "1.5px solid #FDE68A", borderRadius: 9, color: "#374151", padding: "12px", fontSize: 13, outline: "none", fontFamily: "inherit", lineHeight: 1.6 }}
                />
              </div>
            </div>
            ...
            <div style={{ width: 220, display: "flex", flexDirection: "column", gap: 16, flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {STATUSES.map(s => (
                    <button key={s} onClick={() => handleField("status", s)}
                      style={{ padding: "8px 12px", fontSize: 12, fontWeight: selected.status === s ? 700 : 500, borderRadius: 8, border: selected.status === s ? "none" : "1.5px solid #E5E7EB", background: selected.status === s ? statusColor(s).bg : "#fff", color: selected.status === s ? statusColor(s).text : "#6B7280", cursor: "pointer", textAlign: "left", textTransform: "capitalize" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {["high", "medium", "low"].map(p => (
                    <button key={p} onClick={() => handleField("priority", p)}
                      style={{ padding: "8px 12px", fontSize: 12, fontWeight: selected.priority === p ? 700 : 500, borderRadius: 8, border: selected.priority === p ? "none" : "1.5px solid #E5E7EB", background: selected.priority === p ? PRIORITY_COLORS[p].bg : "#fff", color: selected.priority === p ? PRIORITY_COLORS[p].text : "#6B7280", cursor: "pointer", textAlign: "left", textTransform: "capitalize" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {TEAM.map(m => (
                    <button key={m.id} onClick={() => handleField("assignedTo", m.id)}
                      style={{ padding: "8px 12px", fontSize: 12, fontWeight: selected.assignedTo === m.id ? 700 : 500, borderRadius: 8, border: selected.assignedTo === m.id ? "none" : "1.5px solid #E5E7EB", background: selected.assignedTo === m.id ? "#F0EFFE" : "#fff", color: selected.assignedTo === m.id ? "#6366F1" : "#6B7280", cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ width: 22, height: 22, borderRadius: "50%", background: m.color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 10, fontWeight: 800, flexShrink: 0 }}>{m.avatar}</span>
                      {m.name}
                    </button>
                  ))}
                </div>
              ...
              <SideCard label="Category">
                <select value={selected.tag} onChange={e => handleField("tag", e.target.value)}
                  style={{ width: "100%", background: "#F9FAFB", border: "1.5px solid #E5E7EB", borderRadius: 8, color: "#0F1117", padding: "9px 10px", fontSize: 12, fontWeight: 600, outline: "none", cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, background: "#F5F6FA" }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 700, color: "#6B7280" }}>Select a ticket to view details</div>
          <div style={{ fontSize: 12, color: "#9CA3AF", maxWidth: 280, textAlign: "center", lineHeight: 1.6 }}>
