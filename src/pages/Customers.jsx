...
export default function Customers({ tickets }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ width: 300, background: "#fff", borderRight: "1px solid #EAECF0", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "14px 14px 8px" }}>
          <div style={{ position: "relative" }}>
            <span style={{ position: "absolute", left: 11, top: "50%", transform: "translateY(-50%)", color: "#8A9E8A", pointerEvents: "none" }}>
              <SearchIcon />
            </span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search customers..."
              style={{ width: "100%", background: "#FAFAF7", border: "1.5px solid #E5E7EB", borderRadius: 9, color: "#1C2B1C", padding: "9px 12px 9px 34px", fontSize: 15, outline: "none", fontFamily: "inherit" }}
        ...
        <div style={{ padding: "0 14px 6px" }}>
          <span style={{ fontSize: 13, color: "#8A9E8A", fontWeight: 600 }}>{filtered.length} customers</span>
        </div>
              ...
              <div key={c.id} onClick={() => setSelected(c)}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#1C2B1C", marginBottom: 1 }}>{c.name}</div>
                    <div style={{ fontSize: 13, color: "#6B7D6B", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.company}</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    {c.openTickets > 0
                      ...
                      : <span style={{ background: "#EAF0EA", color: "#3D6B3D", fontSize: 12, fontWeight: 600, borderRadius: 6, padding: "2px 6px" }}>✓ clear</span>
                    }
                  </div>
                </div>
              </div>
      ...
      {/* Detail */}
      {selected ? (
        <div style={{ flex: 1, overflowY: "auto", background: "#F7F5F0", padding: 28 }}>
          {/* Profile card */}
          <div style={{ background: "#fff", borderRadius: 14, padding: 24, marginBottom: 20, border: "1px solid #EAECF0", boxShadow: "0 1px 4px rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <div style={{ flex: 1 }}>
              <h2 style={{ margin: "0 0 3px", fontSize: 20, fontWeight: 800, color: "#1C2B1C", letterSpacing: "-0.3px" }}>{selected.name}</h2>
              <div style={{ fontSize: 15, color: "#6B7D6B", marginBottom: 2 }}>{selected.email}</div>
              <div style={{ fontSize: 14, color: "#8A9E8A" }}>🏢 {selected.company}</div>
            </div>
            <div style={{ display: "flex", gap: 12 }}>
              {[
                { label: "Total", value: selected.totalTickets, color: "#3D6B3D", bg: "#EAF0EA" },
                { label: "Open", value: selected.openTickets, color: selected.openTickets > 0 ? "#EF4444" : "#3D6B3D", bg: selected.openTickets > 0 ? "#FEF2F2" : "#EAF0EA" },
                { label: "Resolved", value: selected.totalTickets - selected.openTickets, color: "#6B7D6B", bg: "#FAFAF7" },
              ].map(s => (
                <div key={s.label} style={{ background: s.bg, borderRadius: 10, padding: "10px 16px", textAlign: "center", minWidth: 64 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#8A9E8A", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Tickets */}
          <div style={{ fontSize: 13, fontWeight: 700, color: "#8A9E8A", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Ticket History</div>
          {customerTickets.length === 0 ? (
            <div style={{ color: "#D1D5DB", textAlign: "center", padding: 48, fontSize: 16 }}>No tickets found</div>
          ) : (
            customerTickets.map(t => {
              const tc = TAG_COLORS[t.tag] || { bg: "#F0EDE6", text: "#6B7D6B", dot: "#8A9E8A" };
              return (
                <div key={t.id} style={{ background: "#fff", border: "1px solid #EAECF0", borderRadius: 12, padding: "14px 16px", marginBottom: 10, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: "#1C2B1C" }}>{t.subject}</span>
                    <span style={{ fontSize: 13, color: "#8A9E8A" }}>{t.date}</span>
                  </div>
                  <p style={{ margin: "0 0 10px", fontSize: 14, color: "#6B7D6B", lineHeight: 1.55 }}>{t.body.slice(0, 120)}…</p>
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <span style={{ background: t.status === "resolved" ? "#EAF0EA" : "#FEF2F2", color: t.status === "resolved" ? "#3D6B3D" : "#EF4444", fontSize: 12, fontWeight: 600, borderRadius: 6, padding: "2px 8px" }}>{t.status}</span>
                    {t.replies.length > 0 && <span style={{ fontSize: 12, color: "#8A9E8A", padding: "2px 0" }}>💬 {t.replies.length} repl{t.replies.length === 1 ? "y" : "ies"}</span>}
                  </div>
                </div>
              );
            })
